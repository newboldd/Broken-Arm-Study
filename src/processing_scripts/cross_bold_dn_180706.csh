...
if (${#argv} < 1) then
	echo "usage:	"$program" params_file instructions_file scriptdir"
	exit 1
endif
set prmfile = $1
...
while ($k <= $runs)
	...
	if (${?nounpack}) goto FA
	echo		unpack_4dfp -V study$fstd[$k] $patid"_bx" -nx$nx -ny$ny $squeezestr $zflip
	if ($go)	unpack_4dfp -V study$fstd[$k] $patid"_bx" -nx$nx -ny$ny $squeezestr $zflip
	if ($ny != $matrix[1]) then
		echo "Image acquisition not equal to first run"
		crop_4dfp $patid"_bx" -f -y1to$matrix[1] $patid"_b"$irun[$k]
	else
		foreach e ( img hdr ifh img.rec )
			mv $patid"_bx".4dfp.$e $patid"_b"$irun[$k].4dfp.$e
		end
	endif
	if ($status) then
		@ err++
...
FA:
	...
	if ($economy > 2) then
		...
		echo		/bin/rm $patid"_bx".4dfp."*"
		if ($go)	/bin/rm $patid"_bx".4dfp.*
	endif
	if ($economy > 3) then
...
if (${#sefm}) then
	if (! -e sefm/${patid}_sefm_mag_brain.nii.gz) then
		$scriptdir/sefm_pp_DN.csh ${prmfile} ${instructions}	# creates sefm subdirectory
		#sefm_pp_AZS.csh ${prmfile} ${instructions}	# creates sefm subdirectory
		if ($status) exit $status
	else
		echo sefm exists - skipping sefm_pp_DN.csh
	endif
	...
	mv $F[1]	${patid}_mag.nii
	if (${#F} == 2) rm $F[2]	#uses only the first magnitude image and discards the second
	if ($status) exit $status
...
set sefm = ()
set t2w = $patid"_t2w"
@ Et2w = (-e atlas/$patid"_t2w".4dfp.img && -e atlas/$patid"_t2w".4dfp.ifh)
###################################################
