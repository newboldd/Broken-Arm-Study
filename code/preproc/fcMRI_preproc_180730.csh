...
foreach e (img img.rec ifh hdr)
	...
	#ln -s $FSdir/MSC02_FSWB_on_${target:t}_333.4dfp.$e ./${patid1}_FSWB_on_${target:t}_333.4dfp.$e	# symbolically link FS whole brain mask
	if ($status) exit $status
end

if (${?fmtfile}) goto COMPUTE_DEFINED
echo "Creating new format file"
...
if ($status) exit $status
if (-e ${patid}"_b1"${MBstr}"_xr3d".FD) /bin/rm	${patid}"_b1"${MBstr}"_xr3d".FD
touch						${patid}"_b1"${MBstr}"_xr3d".FD
@ k = 1
while ($k <= ${#fcbolds})
	gawk -f $RELEASE/FD.awk $patid"_b"$fcbolds[$k]${MBstr}"_xr3d".ddat >> ${patid}"_b1"${MBstr}"_xr3d".FD
	@ k++
end
if ($?FDthresh) then 
	if (! ${?FDtype}) @ FDtype = 1
	...
	gawk '{c="+";if ($'$FDtype' > crit)c="x"; printf ("%s\n",c)}' crit=$FDthresh ${patid}"_b1"${MBstr}"_xr3d".FD >! $$.format1
	paste $$.format0 $$.format1 | awk '{if($1=="x")$2="x";printf("%s",$2)}' >! ${patid}"_b1"${MBstr}"_xr3d".FD.format
	/bin/rm $$.format[012]
	/bin/mv ${patid}"_b1"${MBstr}"_xr3d".FD.format ../$FCdir/
endif 
popd			# back to $FCdir
...
if (${?FDthresh}) then 
	...
	format2lst ${patid}"_b1"${MBstr}"_xr3d".FD.format >! $$.format2
	paste $$.format1 $$.format2 | gawk '{if($1=="x")$2="x";printf("%s",$2);}' | xargs condense >! $fmtfile

	/bin/rm $$.format1 $$.format2
endif

# exclude short sequences of included frames
if (${?min_consecutive_frames}) then
	@ k = 1
	while ($k < $min_consecutive_frames)
		set searchstr = x
		set repstr = x
		@ j = 1
		while ($j <= $k)
		...
		set searchstr = ${searchstr}x
		set repstr = ${repstr}x
		set fmt = `format2lst -e $fmtfile | sed "s/$searchstr/$repstr/g"`
		set fmt = `echo $fmt | sed "s/$searchstr/$repstr/g" | xargs condense`
		echo $fmt > $fmtfile
		@ k++
	end
endif
